...
function generateUniqueName($originalName)
{
    ...
    return time() . uniqid() . '-agent-property-images.' . $extension;
}
                ...
                if(!$erroOccure){
                      ...
                      $maxSize = 5 * 1024 * 1024; // 2 MB in bytes
                        ...
                        if(count($imagePaths)>0){
                               ...
                               $validYoutubeUrls=$ebedurl;
                                $images = "SELECT * FROM propertyimages WHERE pid=$pid;";
                                    ...
                                    for ($i=0; $i < count($imagePaths) ; $i++) { 
                                        $targetPath = '../admin/uploads/' . $namesImages[$i];
                                        $uid =  $uid = $_SESSION['agentID'];
                                        $path =$namesImages[$i];
                            ...
                            $con->query($sql);
                            propertyStatus($con, $uid, $pid, "UPDATE `complete` SET `4` = '1' WHERE `complete`.`uid` = $uid AND `pid`=$pid ;");
...
<body>
            ...
            <div class="full-row   rounded bg-white" style="margin-top: 50px; margin-left: 25px; margin-right: 25px;">
                <form method="post" enctype="multipart/form-data">
                    <div class="d-flex justify-content-between">
                        ...
                        <h2 class="mt-2 text-center  border border-info p-3 text-info " style="border-radius: 50%;">4</h2>
                    </div>
    ...
    .uploaded-image {
        width: 150px; /* Set the fixed width of the images */
        height: 150px; /* Set the fixed height of the images */
...
<div class="container rounded">
    <div class="rounded-top p-4 bg-white " style="background-color:white;">
        <div class="row row col-md-12 col-sm-12 col-lg-12">
            <div class="col-lg-2 col-sm-12 col-md-3 text-center text-sm-start" style="color: dark; font-weight: 900; font-size:20px;">
                Property Images and Videos
            </div>
            <div class="col-lg-9 col-sm-12 col-md-8 text-center text-sm-start">
                <div class="form-group">
                       <input type="file" hidden class="form-control bg-white my-2 text-dark" name="imgfiles[]" id="files" multiple="multiple" accept="image/jpeg, image/png, image/jpg" placeholder="Enter Property Title" style="background-color:white;" />
                    ...
                    <div class="col-lg-12 dropzone image-preview-container">
                        ...
                        <div id="imagePreviewContainer" class="mt-3 row"></div> <!-- Container for image previews -->
                    </div>
                    ...
                    <div class="col-xl-12" id="dynamic_field">
                        <div class="form-group row">
                            <label name="add" id="you" class="col-lg-1 bg-white " style="color:red;font-size:40px;"><i class="bi bi bi-youtube"></i></label>
                            <div class="col-lg-8">
                                <input type="url" class="form-control my-2 bg-white text-light" name="youtub" placeholder="Add Youtube video link" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="container-fluid pt-4 px-4 bg-white">
                <div class="rounded-top p-4 bg-white">
                    <div class="row col-md-12 col-sm-12 col-lg-12 bg-white ">
                        <div class="col-md-4 col-sm-4 col-lg-4 text-center " style="color: white; font-weight: 900; font-size:20px;">
                        ...
                        <div class="col-md-4 col-sm-4 col-lg-4 text-center " style="color: white; font-weight: 900; font-size:20px;">
                            <button class="btn btn-outline-danger"> <a href="-add-pro-info.php?PID=<?php echo $pid; ?>">Skip</a></button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#add').click(function() {
                i++;
                $('#dynamic_field').append('<div class="form-group my-2 row" id="row' + i + '"><label  name="add" id="' + i + '" class="col-lg-1 bg-white btn_remove " style="color:red;font-size:40px;"><i class="bi bi bi-youtube"></i></label><div class="col-lg-8"><input type="url" class="form-control my-2 bg-white text-light" name="youtub[]"  placeholder="....." /></div><label  name="add" id="' + i + '" class="col-lg-1 bg-white btn_remove " style="margin-top:10px;font-size:20px;"><i class="bi bi-file-minus-fill"></i></label></div>');
            });
            ...
            for (let i = 0; i < selectedImages; i++) {
                ...
                reader.onload = function(e) {
                    $('#imagePreviewContainer').append(
                        '<div class="col-md-3"><img src="' + e.target.result + '" class="img-thumbnail img-preview" style="width: 300px; height: 100px;"></div>'
                    );
                };
            }
        });
