...
if (isset($_POST['login'])) {
	...
	if (!empty($user) && !empty($pass) && !empty($role)) {
		// echo $user . $pass . $role;
		$query = "SELECT * FROM user WHERE email='$user' AND pass='$pass' AND role='$role' and active='2'";
		...
		if ($num_row == 1) {
			...
			if ($role === 'admin') {
				...
				$_SESSION['adminImg'] = $row['img'];
				header("location:./admin/");
			} else if ($role === 'agent') {
				$active = (int)trim($row['active']);
				$varified = (int)trim($row['varified']);
				$role = (string)trim($row['role']);
				if ($varified === 1) {
					$_SESSION['agentID'] = $row['id'];
					$_SESSION['agentName'] = $row['name'];
					$_SESSION['agentEmail'] = $row['email'];
					$_SESSION['agentImg'] = $row['img'];
