...
if (isset($_POST['reg'])) {
    ...
    if (!$name) {
        $error = "<p class='alert alert-warning'>Please Enter  Status</p>";
    } else if (!$title) {
        $error = "<p class='alert alert-warning'>Please Enter  Title</p>";
    } else if (!$description) {
        $error = "<p class='alert alert-warning'>Please Enter  Description</p>";
        ...
        $uimage = time() . "news" . "." . $fileExtension;
        $temp_name  = $_FILES['a_img']['tmp_name'];
        move_uploaded_file($temp_name, "./common/user/$uimage");

        if (!empty($name) && !empty($title) && !empty($description) && !empty($uimage)) {
            // Use prepared statements to prevent SQL injection
            // Prepare the SQL statement with parameterized query
            $stmt = $con->prepare("INSERT INTO `news` (`title`, `description`, `img`, `date`, `status`) VALUES (?, ?, ?, CURRENT_TIMESTAMP(), ?)");

            // Bind the parameters
            $stmt->bind_param("ssss", $title, $description, $uimage, $name);
                        ...
                        <div class="col-12 col-sm-8 col-md-6 col-lg-5 col-xl-5">
                            ...
                            <div class=" rounded p-4 p-sm-5 my-2 " style="background-color:white">
                                ...
                                <form method="post" enctype="multipart/form-data">
                                   ...
                                   <select class="form-control bg-white my-2" type="text" style="background-color: #191C24;height:50px; color: black; font-family: sans-serif;" name="name">
                                    ...
                                    <div class="form-group">
                                        <input class="form-control bg-white my-2" type="text" style="background-color: #191C24;height:50px; color: black; font-family: sans-serif;" placeholder="Title" name="title">
                                    </div>
                                    <div class="form-group">
                                        <textarea class="form-control bg-white" name="description" id="exampleFormControlTextarea1" maxlength="450" placeholder="Description" style="background-color: #191C24;color: black;" rows="10"></textarea>
                                    </div>
